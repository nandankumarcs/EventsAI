    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Every handled message must include clear in-app next actions the user can take now.\n"
                "Use only supported actions such as selecting by visible position/title, confirming with yes/no, or refining filters by city/date/sport/genre.\n"
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Return a concise user-facing message in all handled actions.\n"
                "Every handled message must include clear in-app next actions the user can take now.\n"
                "Use only supported actions such as selecting by visible position/title, confirming with yes/no, providing the requested user field, or refining filters by city/date/sport/genre.\n"
                "Do not invent listing codes."
            ),
            ("user", "{payload}"),
        ]
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "When there is no meaningful planning context yet (no active filters, no pending booking, and no visible latest results), classify simple greetings/openers as meta_help.\n"
                ...
                "For greeting/opening turns with no planning context, message must provide actionable next-step guidance aligned to this flow (event type, city, date/time, genre/sport, or booking intent), ideally with a few short example prompts.\n"
                "Supported domains in this assistant are only movies and sports.\n"
                "Never suggest or mention unsupported domains such as concerts, music gigs, theatre, flights, hotels, or restaurants.\n"
                "When giving example prompts, include only movie or sports examples.\n"
                "Keep suggestions strictly in-app and context-safe: do not mention unsupported geographies or invented cities.\n"
                "Do not suggest cities like London or any city not present in current thread context.\n"
                "If no concrete city is grounded in current context, use generic phrasing such as 'in your city' instead of naming a city.\n"
                "Every message must include 1-3 concrete in-app next actions the user can take right now.\n"
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "When the thread has no meaningful planning context yet (no active filters, no pending booking, and no visible latest results), treat greetings or generic openers such as 'hi', 'hello', 'hey', 'good morning', 'start', or similar as meta_help, not task_continue.\n"
                "For those fresh-start greeting/meta turns, set search_domains to [], set should_keep_results to false, set should_try_booking_agent to false, and keep resolver booleans false unless the user also provided concrete planning constraints.\n"
                "For those fresh-start greeting/meta turns, message must include actionable next-step guidance aligned to the booking flow, for example asking for event type, city, date/time, genre, sport, or booking intent.\n"
                "Good style for that message: one short friendly line plus 2-4 concise example prompts the user can send next.\n"
                "Supported domains in this assistant are only movies and sports.\n"
                "Never suggest or mention unsupported domains such as concerts, music gigs, theatre, flights, hotels, or restaurants.\n"
                "When giving example prompts, include only movie or sports examples.\n"
                "Keep suggestions strictly in-app and context-safe: do not mention unsupported geographies or invented cities.\n"
